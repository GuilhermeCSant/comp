...
def calculate_differences_merged(df_merged, common_numeric_cols, key_columns, common_cols):
    ...
    rows_only_1 = df_comp[df_comp['Status_Linha'] == 'Apenas Plan1'].shape[0]; rows_only_2 = df_comp[df_comp['Status_Linha'] == 'Apenas Plan2'].shape[0]; rows_both = df_comp[df_comp['Status_Linha'] == 'Ambas Planilhas'].shape[0]
    ...
    for col in common_numeric_cols:
        ...
        if col1_name in df_comp.columns and col2_name in df_comp.columns:
             ...
             total_changes['Aumentou'] += counts.get('Aumentou', 0); total_changes['Diminuiu'] += counts.get('Diminuiu', 0); total_changes['Igual'] += counts.get('Igual', 0); total_changes['Dados Ausentes'] += mask_nan_original.sum()
            ...
            try:
                ...
                with tab_detail:
                    ...
                    if not df_comparison.empty:
                        ...
                        try:
                            if len(key_columns) == 1: df_comparison['_selectbox_key'] = df_comparison[key_columns[0]].astype(str)
                            else: df_comparison['_selectbox_key'] = df_comparison[key_columns].apply(lambda row: ' | '.join(row.values.astype(str)), axis=1)
                            select_options = sorted(df_comparison['_selectbox_key'].unique().tolist()); select_options.insert(0, "Selecione...")
                            ...
                            if selected_key_str != "Selecione...":
                                selected_row_data = df_comparison[df_comparison['_selectbox_key'] == selected_key_str]
                                if not selected_row_data.empty:
                                    ...
                                    elif selected_row.get('Status_Linha') != 'Ambas Planilhas': st.info(f"Linha existe apenas na '{selected_row['Status_Linha']}'."); st.write("**Dados:**"); cols_to_show_single = [c for c in df_comparison.columns if not ('_AbsDiff' in c or '_PercDiff' in c or '_Indicador' in c or '_selectbox_key' in c)]; st.dataframe(selected_row[cols_to_show_single].to_frame().T, use_container_width=True)
                                    else: st.info("Linha em ambas, sem col. numéricas comuns."); st.write("**Dados:**"); cols_to_show_no_comp = [c for c in df_comparison.columns if not ('_AbsDiff' in c or '_PercDiff' in c or '_Indicador' in c or '_selectbox_key' in c)]; st.dataframe(selected_row[cols_to_show_no_comp].to_frame().T, use_container_width=True)
                                else: st.error(f"Erro: Não localizou dados para '{selected_key_str}'.")
                            if '_selectbox_key' in df_comparison.columns: df_comparison.drop(columns=['_selectbox_key'], inplace=True)
                ...
                with tab_charts:
                    ...
                    if common_numeric_cols and rows_both > 0:
                        ...
                        if not df_plot_base.empty:
                            ...
                            try:
                                if x_axis_col_name in key_columns and pd.api.types.is_numeric_dtype(df_plot_base[x_axis_col_name]): df_plot_base = df_plot_base.sort_values(by=x_axis_col_name); df_plot_base[x_axis_col_name] = df_plot_base[x_axis_col_name].astype(str)
                                elif x_axis_col_name == '_CombinedKey': df_plot_base = df_plot_base.sort_values(by=x_axis_col_name)
                            except Exception as e: st.warning(f"Não ordenou dados: {e}")
                            ...
                            if perc_col_name in df_plot_base.columns and x_axis_col_name in df_plot_base.columns:
                                ...
                                else: st.info(f"Não há dados de diferença % válidos para '{col_to_visualize}'.")
                            else: st.warning(f"Erro gráfico linha '{col_to_visualize}'.")
                        else: st.info("Sem linhas correspondentes para gráficos.")
                    elif not common_numeric_cols: st.info("Sem colunas numéricas comuns para gráficos.")
                    else: st.info("Sem linhas correspondentes para gráficos.")

            except KeyError as e: st.error(f"Erro Chave Comparação: '{e}'."); st.exception(e)
            except ValueError as ve: st.error(f"Erro Valor Comparação: {ve}."); st.exception(ve)
...
st.sidebar.markdown("App para comparar interativamente planilhas Excel/CSV.")